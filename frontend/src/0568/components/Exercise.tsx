...
const Exercise = () => {
	...
	const getData = async (selected: any) => {
        const timeout = setTimeout(() => {
            if (!exercise) setExercise([]);
            if (!equipment) setEquipment([]);
            if (!target) setTarget([]);
        }, 5000);
		...
		try {
			...
			switch (selected.type) {
				case "Equipment":
					let list = await getListByEquipment(selected.payload);
					setExercise(list || []);
					break;
				case "Target Muscle":
					let list2 = await getListByMusculeTarget(selected.payload);
					setExercise(list2 || []);
					break;
				default:
